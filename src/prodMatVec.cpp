...
NumericVector pMatVec4(MatrixAccessor<T> macc,
  ...
  for (j = 0; j <= m - 4; j += 4) {
    ...
    for (i = 0; i < n; i++) { // unrolling optimization
      i2 = rows[i];
      res[i] += (x[j] * macc[j2][i2] + x[j+1] * macc[j2+1][i2]) +
        (x[j+2] * macc[j2+2][i2] + x[j+3] * macc[j2+3][i2]);
...
NumericVector cpMatVec4(MatrixAccessor<T> macc,
  ...
  for (j = 0; j < m; j++) {
    ...
    for (i = 0; i <= n - 4; i += 4) { // unrolling optimization
      i2 = rows[i];
      tmp += (macc[j2][i2] * x[i] + macc[j2][i2+1] * x[i+1]) +
        (macc[j2][i2+2] * x[i+2] + macc[j2][i2+3] * x[i+3]);
    }
    for (; i < n; i++) {
      i2 = rows[i];
      tmp += macc[j2][i2] * x[i];
