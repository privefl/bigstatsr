...
test <- big_readBM(tmp,
                   ...
                   fun.createBM = BM(descriptor = FALSE))  # default returns a descriptor
```
...
I think it is usually a good idea to use filebacked `big.matrix` objects (not the case here) because you can write them once and attach them later on. Moreover, you have usually a very large amount of available disk space, but a limited amount of RAM or shared memory.<br/>
To do just that, change the parameter `fun.createBM`.

Moreover, using descriptors rather than directly `big.matrix` objects is a good idea.
