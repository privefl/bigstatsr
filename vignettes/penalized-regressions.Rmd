...
Functions `big_spLinReg()` and `big_spLogReg()` automatically perform a procedure similar to cross-validation in order to choose hyper-parameters $\lambda$ and $\alpha$ of the elastic net regularization:
...
- the early stopping criterion is reached, which means that models are getting worse on the inner validation set; you can control this using `nlam.min` (the minimal number of $\lambda$ values to try before stopping) and `n.abort` (the number of $\lambda$ values for which the model is getting worse)

- the 200  $\lambda$ values have been used; you might want to reduce `lambda.min`

It is possible to fit the `K` folds and the sequence of `alphas` in parallel using parameter `ncores`.
...
Here, `"No more improvement"` means that the early stopping criterion have been reached.
...
For large datasets, you might want to decrease `n.abort` (i.e. stop the regularization path quickly) as it is much more smooth and fitting is more and more demanding as we advance along the regularization path (because more and more variables are used in the model).
...
- Functions `big_spLinReg()` and `big_spLogReg()` are not deterministic because folds are chosen randomly. One way that should ensure reproducibility is to define your own folds using parameter `ind.sets`.
