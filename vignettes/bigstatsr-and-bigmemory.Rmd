...
- descriptor
- difference with biganalytics, bigtabulate and bigalgebra
- split-apply-combine (instead of biganalytics)
- memory- and file-mapped and shared

I'm a super fan of the **bigmemory** package. It's by far the most convenient solution for analyzing large genomic data in R on my computer. I've been using it for almost two years now and have also contributed some features. 

At first, package **bigstatsr** was using the `big.matrix` objects. Yet, at some point, I felt the need to become independent of the **bigmemory** package. As package **bigstatsr** will be a central tool of all my work during my thesis, I need to add whatever feature I want whenever I want to. Thus, I reimplemented an object very similar to the filebacked `big.matrix` object, called "FBM" (Filebacked Big Matrix, very original) in this package. These two formats are so similar that you can easily convert (without copying the data) between the two objects.
...
I have been thinking of using the unsigned short type to make a `FBM.code65536` in order to store 65536 different values. I would use it to store some strings in order to implement a "big.data.frame" feature (with no more than 65536 strings in it, unfortunately). Yet, as I'm not needing this right now, I haven't implemented it yet and I'm not sure I will one day.

## Class

A `big.matrix` is basically an S4 class object that stores a pointer to a C++ object (an external pointer). When you restart your R session, this pointer becomes `Nil` and it may makes your session crash. You'll need a different object, a `big.matrix.descriptor` (using `describe()`) which stores enough information to make it possible to create this external pointer again (using `attach.big.matrix()`).
A drawback of this is that you have to often switch between descriptors and `big.matrix` objects.

For FBMs, I use the nice idea of the package [**bigmemoryExtras**](https://github.com/phaverty/bigmemoryExtras). Basically, I use a Reference Class object with active binding. In this object, I store the external pointer *and* the information needed to reattach the pointer to a C++ object. The active binding makes this automatic so that the user never need to use `attach.big.matrix()` or `describe()` anymore (and no more session crash!). 

What this also mean is that you can now serialize a FBM (for example, saving it in an rds file with `saveRDS()` or using it in a parallel algorithm). With a standard `big.matrix` object, you'll need to pass the descriptor object in paralell algorithms:
