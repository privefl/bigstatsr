...
for (i in 2:length(lam)) {
  printf("\ni = %d\n", i)
  seq_strong_thr <- (2 * lam[i] - lam[i - 1]) * 1.00
  KKT_thr <- lam[i] * 1.00
  ...
  repeat {
    ...
    # step c
    printf("c - "); print((bad_KKT_c <- which(!in_A & in_S & (cp > KKT_thr))))
    if (length(bad_KKT_c)) {
    ...
    # step d
    printf("d - "); print((bad_KKT_d <- which(!in_A & !in_S & (cp > KKT_thr))))
    if (length(bad_KKT_d)) {
      # Add these predictors
      # sorting by appearance? and avoing recopying the matrix
      in_A[bad_KKT_d] <- TRUE
    ...
    in_A[all_betas != 0] <- TRUE # really that?
    b[i] <- sum(betas.new != 0)
    break
  }
}
...
print(system.time(mod <- glmnet(X, y.simu, lambda = lam)))
...
print(system.time(mod2 <- biglasso(X.big, y.simu, lambda = lam)))
